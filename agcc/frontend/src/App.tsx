...
function Mission() {
  ...
  const refresh = useCallback(async (target: MissionMode) => {
    ...
    if (state.resolution_required) {
      if (!fetchingResolution.current[target]) {
        ...
        client.request<Resolution>('/mission/resolution').then(setResolution).catch(() => { fetchingResolution.current[target] = false })
      }
    } else {
  ...
  useEffect(() => {
    ...
    initializing.current.clear(); setRuntimes({}); setResolution(null); fetchingResolution.current = {}
