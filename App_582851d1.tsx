...
function validateDraft(draft: Draft): string[] {
  ...
  if (!range || draft.frequency < range[0] || draft.frequency > range[1]) errors.push(`${draft.band}-band requires ${range?.[0] ?? '?'}ΓÇô${range?.[1] ?? '?'} GHz; ${draft.frequency} GHz is contradictory.`)
                  ...
                  <div className="field-grid">
                    <Field label="Band">
                      <select value={draft.band} onChange={(event) => chooseBand(event.target.value)}>
                        <option value="X">X [8ΓÇô12 GHz]</option>
                        <option value="S">S [2ΓÇô4 GHz]</option>
                        <option value="Ka">Ka [26.5ΓÇô40 GHz]</option>
                      </select>
                    </Field>
...
function AnomalyChat({ runtime, refresh, resetToStart }: { runtime: Runtime; refresh: () => void; resetToStart: () => void }) {
  ...
  const send = () => {
    ...
    client.request<AnomalyProposal>('/anomalies/chat', { method: 'POST', body: JSON.stringify({ text: transcript.join('\n') }) }).then((item) => {
      ...
      const reply = item.clarification_questions.join(' ') || `Proposed ${item.intent.anomaly_type ?? 'anomaly'} at multiplier ${item.rate_multiplier ?? 'unresolved'}├ù${interval}${confidence}. Confirm to inject it.`
  ...
  const confirm = async () => {
    ...
    setActionBusy('confirm'); setMessage('Recording anomaly on the branch timelineΓÇª')
    try {
  ...
  const requestReplan = async () => {
    ...
    setActionBusy('replan'); setMessage('Calculating an anomaly-adjusted forward plan. Please wait; this can take time for long missionsΓÇª')
    try {
  ...
  const decide = async (decision: 'approve'|'reject') => {
    ...
    setActionBusy(decision); setMessage(`${decision === 'approve' ? 'Approving and activating' : 'Rejecting'} the proposed branch planΓÇª`)
    try {
  ...
  return <>
    <section className="anomaly-workbench panel">
      ...
      <div className="anomaly-chat"><textarea value={text} onChange={(event) => setText(event.target.value)} placeholder={`Example: ${runtime.state.stations.find((item) => item.classification === 'approved')?.name ?? 'the next station'} has severe link degradation`}/><button disabled={!text.trim() || busy || Boolean(actionBusy) || watson?.reachable === false} onClick={send}>{busy ? 'ContactingΓÇª' : 'Ask Groq'}</button></div>
      <p className="workflow-message">{message}</p>
      {actionBusy && <div className="action-progress" role="progressbar" aria-label={`${actionBusy} in progress`}><span/><b>{actionBusy === 'replan' ? 'Computing pass capacity and validating routeΓÇª' : `${actionBusy} in progressΓÇª`}</b></div>}
      {proposal && <div className="proposal-card"><b>{proposal.status.replaceAll('_',' ')}</b><span>{proposal.intent.anomaly_type ?? 'unresolved'} ┬╖ {stationName(proposal.intent.station_id)} ┬╖ multiplier {proposal.rate_multiplier ?? 'needs clarification'}├ù<AssumptionMark reason="Groq proposed the effect; deterministic bounds validate the multiplier."/></span><button disabled={proposal.status !== 'pending' || Boolean(actionBusy)} onClick={confirm}>{actionBusy === 'confirm' ? 'ConfirmingΓÇª' : 'Confirm branch injection'}</button><button disabled={Boolean(actionBusy) || (proposal.status !== 'confirmed' && !runtime.events.some((event) => event.event_type === 'anomaly_detected'))} onClick={requestReplan}>{actionBusy === 'replan' ? 'CalculatingΓÇª' : 'Calculate replan'}</button></div>}
      {replan && <div className="proposal-card"><b>Approval required</b><span>Added future contacts: {addedStations.join(', ') || 'No replacement contact'} ┬╖ cost ╬ö {replan.diff?.cost_delta ?? 'not applicable'} ┬╖ remaining shortfall {replan.predicted_shortfall_after_mb.toFixed(2)} MB</span><small>Approve explicitly authorizes the calculated plan cost. If needed, the branch budget ceiling will increase only to that total; the deadline remains hard.</small><button disabled={Boolean(actionBusy) || !replan.diff || replan.predicted_shortfall_after_mb > 1e-9} onClick={() => void decide('approve')}>{actionBusy === 'approve' ? 'ApprovingΓÇª' : 'Approve'}</button><button disabled={Boolean(actionBusy)} onClick={() => void decide('reject')}>{actionBusy === 'reject' ? 'RejectingΓÇª' : 'Reject'}</button></div>}
    </section>
...
function Mission() {
  ...
  const initializeAll = useCallback(async (draft: Draft, currentRevision: number) => {
    ...
    setStatuses({ prediction: 'Building shared authoritative baselineΓÇª', live: 'Waiting for shared baselineΓÇª', branch: 'Waiting for shared baselineΓÇª' })
    setFailures({})
